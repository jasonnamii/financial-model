...
- **하는 것** — "VC-grade 재무모델 빌더.
- **안 하는 것** — 회계장부(→xlsx스킬직접)."
...
- 사용자가 "만들어줘", "추정해줘", "터보로 만들어줘", "build", "create" 같은 표현으로 발동
- 도메인 작업이 필요한 시점
- **안 쓸 때** — 회계장부(→xlsx스킬직접)."
...
| ALLOW | 업계 전문용어(LTV·CAC·MRR·ARR·BEP·IRR·NPV·DCF·EBITDA·CAGR·CAPEX·OPEX·COGS·GMV·SaaS·VC) · 고유명사 · 법조문 |
| CONVERT | 라벨 발견 → 실명·평문 풀어쓰기. 예) "가정→산출→시나리오·TURBO·시나리오 3종·감도매트릭스" → 결과만 노출(시나리오 이름은 평문) / "탭·병렬 Agent" → 본문 비노출 |
| SELF_CHECK | .xlsx·방법론.md 출력 직전에서 자체 스캔. 1개라도 발견 = 차단·재작성. paper-engine cascade 경유 시 INV 13 자동 적용 |
...
| 산출물 | 경로 |
|---|---|
| 주 산출물 | `mnt/outputs/financial-model_{topic}_{YYYY-MM-DD}.md` |
| 형식 | .xlsx로, .md로, 스프레드시트로. |
| 리서치 결과 (해당 시) | `{VAULT}/_skills research/financial-model/{YYYY-MM-DD}_{topic}.md` |
